...
const regex = /<div className="flex items-center gap-1\.5">[\s\S]*?MATIKAN[\s\S]*?<\/button>[\s\S]*?<\/div>/;

const newButtons = `<div className="flex flex-wrap items-center justify-end gap-1.5 sm:gap-2 mt-2 lg:mt-0 w-full lg:w-auto">
                                {/* Start/Pause */}
                                <button
...
code = code.replace(/className="flex items-center justify-between gap-3 bg-slate-900 px-3 py-2\.5 rounded-xl border border-slate-800"/g, 'className="flex flex-col lg:flex-row items-center justify-between gap-3 bg-slate-900 px-3 py-2.5 rounded-xl border border-slate-800"');
fs.writeFileSync('src/components/AdminDashboard.tsx', code);
console.log("Patched AdminDashboard.tsx successfully! (Regex)");
